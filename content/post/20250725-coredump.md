...
author:      "Wayne"
image: ""
tags: ["c", "gdb", "coredump", "dump", "ulimit", "setrlimit"]
categories: ["Tech"]
---
...
通过 _/etc/security/limits.conf_ 设置用户级别的 core 文件生成限制（对 login shell 生效）：
...
## core dump 通过 | 开头（管道符） 被 Apport 或 systemd 接管分析处理
...
为了让这个设置在系统每次启动后都自动生效，你就需要把它写进 `/etc/sysctl.conf`。
...
### demos(不同的 distribution 的 core_pattern 默认值不同)
...
```bash
root@ubuntu:/home/hugo_site# cat /proc/sys/kernel/
Display all 122 possibilities? (y or n)
root@ubuntu:/home/hugo_site# cat /proc/sys/kernel/core_pattern
|/usr/share/apport/apport -p%p -s%s -c%c -d%d -P%P -u%u -g%g -F%F -- %E
root@ubuntu:/home/hugo_site#
```
